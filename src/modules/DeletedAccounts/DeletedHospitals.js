...
export default function DeletedHospitals() {
  const { t } = useTranslation();
  const [hasError, updateHasError] = useState({
    status: false,
    message: "null",
  });
  const [deletedHospitalsDetails, updateDeletedHospitalsDetails] = useState([]);
  ...
  const handleGetDeletedHospitalsAccounts = () => {
    updateLoading(true);
    ...
    getDeletedAccounts(
      ...
      (success) => {
        updateLoading(false);
        updateDeletedHospitalsDetails(success.data.users);
        updatePagination({
  ...
  const handleAcceptRejectHospitals = (data) => {
    ...
    acceptRejectUser(
      ...
      (success) => {
        updateLoading(false);
        handleGetDeletedHospitalsAccounts();
  ...
  return (
    <>
      ...
      <Table
        ...
        columns={columns}
        dataSource={deletedHospitalsDetails}
        loading={loading}
