...
export function getHospitalDetails(data, onSuccess, onFail) {
  const { type } = data;
  delete data.type;
...
export function getAllPackages(data, onSuccess, onFail) {
  const path = PATHS(`/dashboard/packages/submissions`, data);
  getResource(path, onSuccess, onFail);
}
export function approveRejectHospitalPackage(data, onSuccess, onFail) {
  const { id, status } = data;
  const path = PATHS(`/dashboard/packages/submissions/${id}/${status}`);
  delete data.status;
  delete data.id;
  postResource(path, data, onSuccess, onFail);
}
